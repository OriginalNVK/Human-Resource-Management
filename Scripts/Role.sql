...
-------------------------------------------------------------------------
-- Gán quyền cho CTSV
GRANT SELECT, INSERT, UPDATE, DELETE ON QLDH_SINHVIEN TO NV_CTSV;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON QLDH_DANGKY TO NV_CTSV;
...
-- VIEW SCHEDULE CỦA NHAN VIEN THUOC KHOA
CREATE OR REPLACE VIEW PDB_ADMIN.VIEW_SCHEDULE_OF_EMPLOYEES AS
SELECT mh.MAMH, mh.MAHP, hp.TENHP, hp.SOTC, hp.STLT, hp.STTH, mh.HK, TO_CHAR(mh.NAM) || '-' || TO_CHAR(mh.NAM + 1) AS NAMHOC
FROM pdb_admin.QLDH_MONHOC mh JOIN pdb_admin.QLDH_HOCPHAN hp ON mh.MAHP = hp.MAHP
JOIN pdb_admin.QLDH_NHANVIEN nv ON nv.MANV = mh.MAGV
...
-- Xem điểm cho Sinh viên (mới upd)
CREATE OR REPLACE VIEW QLDH_VIEW_SCORE_SV AS
SELECT mh.MAHP, TENHP, mh.HK, TO_CHAR(mh.NAM) || '-' || TO_CHAR(mh.NAM + 1) AS NAMHOC, NVL(dk.DIEMTH, 0) AS DIEMTH, NVL(dk.DIEMQT, 0) AS DIEMQT, NVL(dk.DIEMCK, 0) AS DIEMCK, NVL(dk.DIEMTK, 0) AS DIEMTK
FROM pdb_admin.QLDH_DANGKY dk
JOIN pdb_admin.QLDH_MONHOC mh ON dk.MAMH = mh.MAMH
JOIN pdb_admin.QLDH_HOCPHAN hp ON mh.MAHP = hp.MAHP
JOIN pdb_admin.QLDH_SINHVIEN sv ON sv.MASV = dk.MASV
WHERE sv.MASV = SYS_CONTEXT('USERENV','SESSION_USER');

GRANT SELECT, UPDATE, DELETE, INSERT ON QLDH_VIEW_COURSE_INFO_SV TO SV;
GRANT SELECT, UPDATE, DELETE, INSERT ON QLDH_VIEW_REGISTERED_INFO_SV TO SV;
GRANT SELECT, UPDATE, DELETE, INSERT ON QLDH_DANGKY TO SV;
GRANT SELECT ON QLDH_VIEW_SCORE_SV TO SV;

GRANT SELECT, UPDATE, DELETE, INSERT ON QLDH_MONHOC TO NV_PDT;
GRANT SELECT ON QLDH_NHANVIEN TO NV_PDT;
GRANT SELECT ON QLDH_DONVI TO NV_PDT;
GRANT SELECT ON QLDH_HOCPHAN TO NV_PDT;

GRANT SELECT, UPDATE, DELETE, INSERT ON QLDH_MONHOC TO NV_CTSV;
GRANT SELECT ON QLDH_NHANVIEN TO NV_CTSV;
GRANT SELECT ON QLDH_DONVI TO NV_CTSV;
GRANT SELECT ON QLDH_HOCPHAN TO NV_CTSV;

-- Grant INSERT, DELETE, UPDATE (except TINHTRANG) on QLDH_SINHVIEN to NV_CTSV
GRANT INSERT, DELETE, UPDATE (MASV, HOTEN, PHAI, NGSINH, DCHI, DT, KHOA) ON QLDH_SINHVIEN TO NV_CTSV;
...
CREATE OR REPLACE VIEW PDB_ADMIN.QLDH_VIEW_SUBJECTS_BY_GV AS
SELECT DISTINCT HP.MAHP, HP.TENHP, MH.HK, MH.NAM
FROM PDB_ADMIN.QLDH_MONHOC MH
JOIN PDB_ADMIN.QLDH_HOCPHAN HP ON MH.MAHP = HP.MAHP
WHERE MH.MAGV = SYS_CONTEXT('USERENV','SESSION_USER');

-- Grant SELECT permission to GV role
GRANT SELECT ON PDB_ADMIN.QLDH_VIEW_SUBJECTS_BY_GV TO NV_GV;
...
-- Create simple view for all subjects
CREATE OR REPLACE VIEW PDB_ADMIN.QLDH_VIEW_ALL_SUBJECTS AS
SELECT 
    MAHP,
    TENHP
...
-- Grant SELECT permission to NV_PKT role
GRANT SELECT ON PDB_ADMIN.QLDH_VIEW_ALL_SUBJECTS TO NV_PKT;
...
GRANT SELECT ON PDB_ADMIN.QLDH_VIEW_SUBJECT_PDT TO NV_PDT;
GRANT SELECT ON PDB_ADMIN.QLDH_VIEW_SUBJECT_PDT TO NV_CTSV;
