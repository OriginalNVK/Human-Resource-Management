        ...
        private void LoadStudents()
        {
            try
            {
			   ...
			   .ConnectionString;
                ...
                cmd.Connection = conn;
                cmd.CommandText = "SELECT * FROM ( SELECT a.MANV \"HoTen\", a.PHAI \"NGSINH\", a.LUONG \"PHUCAP\", a.DT \"VAITRO\", a.MADV , rownum r__ FROM ( SELECT * FROM SYS.QLDH_NHANVIEN ORDER BY MAGV ASC ) a WHERE rownum < ((" + currFrom.ToString() + " * " + pageSize.ToString() + ") + 1 ) ) WHERE r__ >= (((" + currFrom.ToString() + "-1) * " + pageSize.ToString() + ") + 1)";
