        ...
        private void LoadStudents()
        {
            if (GiaoVien != "")
            {
                try
                {
                    ...
                    count.Text = dgvStudents.RowCount.ToString() + "/" + RoleManager.limited;
            ...
            else
            {
                try
                {
                    ...
                    OracleCommand cmd = new OracleCommand();
                    cmd.Connection = conn;
                    cmd.CommandText = "SELECT A.MSSV \"Student ID\", A.HO_TEN_SV \"Name\", B.GIUAKI \"Mid term\", B.CUOIKI \"Final term\", B.DTB \"Average\" FROM ( SELECT MSSV, HO_TEN_SV FROM SYSTEM.SINHVIEN ) A, ( SELECT MALOPHP, MSSV, GIUAKI, CUOIKI, DTB FROM SYSTEM.KETQUA WHERE MALOPHP='" + RoleManager.ClassSectionID + "' ) B WHERE A.MSSV = B.MSSV";
                    cmd.CommandType = CommandType.Text;
                    conn.Open();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (isSelected)
            {
                try
                {
                    ...
                    OracleCommand cmd = new OracleCommand();
                    cmd.Connection = conn;
                    cmd.CommandText = "UPDATE SYSTEM.KETQUA SET GIUAKI=" + txtFinal.Text.Replace(',', '.') + ", CUOIKI=" + txtFinal.Text.Replace(',', '.') + ", DTB=" + txtAver.Text.Replace(',', '.') + " WHERE MSSV='" + lbMSSV.Text + "' AND MALOPHP='" + RoleManager.ClassSectionID + "'";
                    cmd.CommandType = CommandType.Text;
                    conn.Open();
        ...
        private void pbDelete_Click(object sender, EventArgs e)
        {
            if (isSelected)
            {
                try
                {
                    ...
                    OracleCommand cmd = new OracleCommand();
                    cmd.Connection = conn;
                    cmd.CommandText = "DELETE FROM SYSTEM.KETQUA WHERE MSSV='"+ lbMSSV.Text + "' AND MALOPHP='" + RoleManager.ClassSectionID + "'";
                    cmd.CommandType = CommandType.Text;
                    conn.Open();
